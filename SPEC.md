...
- Provider invocation (direct — loop and providers run in the same environment)
  ...
  - Per-iteration timeout (configurable via `ALOOP_PROVIDER_TIMEOUT`, default 2 hours) — kill child process tree on timeout; only acts as a safety net for truly hung processes
