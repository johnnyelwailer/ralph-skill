# Issue #173: Add concurrent_cap failure classification with 2-min hard cap in both shells
...
**This issue is already implemented.** All acceptance criteria have been verified against the current branch state. The child loop should confirm each criterion, then close the issue as done.

**Verification summary (as of refinement):**
- `loop.sh` line 1034: `classify_provider_failure()` returns `concurrent_cap` for `cannot launch inside another session` ✓
- `loop.ps1` line 1393: `Classify-ProviderFailure` returns `concurrent_cap` via `.ToLowerInvariant()` match ✓
- `loop.sh` lines 1076–1077: `update_provider_health_on_failure()` sets `cooldown_secs=120` directly for `concurrent_cap`, bypassing `get_provider_cooldown_seconds()` ✓
- `loop.ps1` line 1429: `Update-ProviderHealthOnFailure` uses inline ternary `if ($reason -eq 'concurrent_cap') { 120 } else { Get-ProviderCooldownSeconds ... }` ✓
- Both shells set status to `cooldown` (not `degraded`) for `concurrent_cap` via the `cooldown_secs -gt 0` branch ✓
- Both shells emit `provider_cooldown` log event with `reason` and ISO-8601 `cooldown_until` ✓
- `get_provider_cooldown_seconds()` / `Get-ProviderCooldownSeconds` is NOT called for `concurrent_cap` ✓
- `auth` failures still produce `degraded` status in both shells ✓
- All other failure types (`rate_limit`, `timeout`, `unknown`) unaffected ✓
...
Only these two files may be modified (if any gap is found):
- `aloop/bin/loop.sh`
- `aloop/bin/loop.ps1`
