...
- [ ] Proof is a first-class phase in the completion finalizer, with its own `PROMPT_proof.md` template
- [ ] Default continuous cycle remains: plan → build × 5 → qa → review (8-step), and the finalizer ends with proof
...
- [ ] Both `loop.ps1` and `loop.sh` support proof in finalizer resolution
...
- [ ] Default continuous cycle includes QA: plan → build × 5 → qa → review (8-step)
...
3. Write `requests/dispatch.json` → runtime:
   ...
   - Compiles child's `loop-plan.json` with implementation cycle (plan-build-qa-review) and proof in finalizer
...
| `loop-plan.json` | Orchestrator: single scan prompt cycle. Children: plan-build-qa-review cycle + finalizer proof |
...
The default `plan → build × 5 → qa → review` cycle is a **configurable, runtime-mutable pipeline of agents**. This cycle is the default configuration, compiled into `loop-plan.json` at session start. Proof runs in finalizer by default. Pipelines are fully customizable via agent YAML definitions.
...
- Default pipeline (plan → build × 5 → qa → review, with proof in finalizer) is generated if no config exists — backward compatible
