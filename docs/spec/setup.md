...
- Autonomy levels as setup output
...
2. **Autonomy level** — `attended | supervised | autonomous | fully_autonomous` (see §Autonomy levels). Default depends on maturity signal from Phase 1.
...
- **`aloop/config.yml`** — project-level configuration consumed by the daemon: tracker adapter, status/label maps, provider chain, validation commands, safety rules, privacy policy, budget cap, autonomy level.
...
Autonomy level is chosen at interview time and stored in `aloop/config.yml`. It gates what the orchestrator is allowed to do without human intervention. Full semantics are in `orchestrator.md` §Autonomy levels; this section covers the setup-facing view.

| Level | Orchestrator behavior | When setup recommends it |
|---|---|---|
| `attended` | Halts before decompose, dispatch, and every merge; human approves each | First time using aloop on this project; sensitive/high-stakes codebase |
| `supervised` | Proceeds through decompose + dispatch autonomously; halts before each merge | Most projects after a first clean loop; default for brownfield |
| `autonomous` | Proceeds end-to-end; human intervenes only on diagnose outputs requiring it | Mature projects with weeks of clean runs; good CI + tests |
| `fully_autonomous` | Same as autonomous plus raising scheduler thresholds without asking | Not recommended for production until the system has demonstrated reliability |

The setup interview defaults to `supervised` for most projects. It defaults to `attended` when discovery signals high risk (no tests baseline, production-deployed, sensitive data declared). It never defaults to `fully_autonomous`.

Autonomy is a **daemon-enforced policy**, not a prompt-level suggestion. Changing it post-setup requires a `PATCH /v1/projects/:id/config` call with appropriate authorization.
...
| **Orchestrator** (`orchestrator.md`) | `agent/trunk` branch, autonomy level, tracker adapter, spec files to decompose | Decompose runs against missing / stale spec, produces bad Epics |
