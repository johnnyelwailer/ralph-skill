...
- links, screenshots, images, audio, spoken notes, voice notes, transcripts, videos, documents, logs, or pasted observations
...
The composer is the preferred path for raw intent because early ideas are usually incomplete and often multimodal. It can accept text, links, screenshots, spoken notes, voice notes, videos, documents, logs, and pasted threads; ask clarifying questions; infer scope; prepare a source plan; propose a budget; and start a research run or monitor after the daemon validates the request. The resulting state is still an `IncubationItem`, `ResearchRun`, `ResearchMonitor`, `OutreachPlan`, or `IncubationProposal`, with media and transcripts preserved as artifacts.
