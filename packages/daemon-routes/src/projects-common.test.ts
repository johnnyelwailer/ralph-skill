...
describe("projectResponse", () => {
  ...
  test("maps all required fields with _v=1 envelope", () => {
    const result = projectResponse(makeProject());
  ...
  test("uses canonical snake_case field names", () => {
    ...
    const result = projectResponse(makeProject({ id: "x", absPath: "/a", name: "A", status: "setup_pending" as const })) as Record<string, unknown>;
  ...
  test("returns a plain object that is JSON serializable", () => {
    const result = projectResponse(makeProject({ id: "x", name: "A", status: "archived" as const }));
    // Should be JSON serializable (can round-trip through JSON.parse/stringify)
    expect(JSON.parse(JSON.stringify(result))).toEqual(result);
    // Should have all expected keys
    expect(Object.keys(result).sort()).toEqual(
      ["_v", "id", "abs_path", "name", "status", "workspace_ids", "added_at", "last_active_at", "updated_at"].sort(),
    );
  });

  test("passes through all status values from VALID_STATUSES", () => {
    for (const status of VALID_STATUSES) {
      const result = projectResponse(makeProject({ status }));
      expect(result.status).toBe(status);
    }
  });

  test("returns a new object each call (no mutation risk)", () => {
    const result1 = projectResponse(makeProject());
    const result2 = projectResponse(makeProject());
