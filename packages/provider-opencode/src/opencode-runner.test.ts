...
async function runOk(input: {
  ...
  return runOpencodeCli(partial as Parameters<typeof runOpencodeCli>[0]);
}
...
describe("runOpencodeCli", () => {
  ...
  test("returns ok:true with text when exit code is 0", async () => {
    ...
    expect((result as { ok: true; text: string }).text).toBe("hello from opencode");
  });
  ...
  test("returns ok:false with exitCode, stdout, stderr when exit code is non-zero", async () => {
    ...
    const ng = result as { ok: false; exitCode: number | null; stdout?: string; stderr?: string; timedOut?: boolean };
    expect(ng.exitCode).toBe(1);
    expect(ng.stdout).toBe("some output");
    expect(ng.stderr).toBe("error message");
    expect(ng.timedOut).toBeUndefined();
  });
