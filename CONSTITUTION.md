...
1. **HARD RULE: loop.sh/loop.ps1 must shrink, never grow.** The loop scripts are dumb runners — they iterate phases, invoke providers, and write status/logs. **Nothing may be added to loop.sh or loop.ps1.** Any PR that touches these files must reduce their line count. If an issue requires new behavior that seems like it belongs in the loop, it MUST be moved to the runtime (`process-requests.ts`, `orchestrate.ts`, or a new CLI command) instead. The loop scripts are already far too large — every touch is an opportunity to extract logic into the runtime. No business logic, no GH calls, no convention-file processing, no new functions, no exceptions.
2. **Inner loop / runtime separation.** The loop may run in a container where the aloop CLI is unavailable. All host-side operations (GH, steering, dashboard, request processing) belong in the runtime (`aloop` CLI / `process-requests.ts`), never in the loop scripts. Directory setup, post-phase validation, artifact management, and any conditional logic based on phase results belong in the runtime.
