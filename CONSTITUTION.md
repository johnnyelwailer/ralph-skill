...
4. **Agents are untrusted.** The aloop runtime is the single trust boundary. Agents never call GH APIs, network endpoints, or the `gh` CLI directly. Agents interact with the runtime only through defined contracts (currently file-based: `requests/*.json`, `queue/*.md` — may evolve to CLI/MCP). The contract mechanism may change, but the trust boundary does not.
