...
4. **Agents are untrusted.** The aloop runtime is the single trust boundary. Agents never call GH APIs, network endpoints, or the `gh` CLI directly. Agents interact with the runtime only through file-based contracts (`requests/*.json`, `queue/*.md`) — never by calling the aloop CLI or any other tool directly.
