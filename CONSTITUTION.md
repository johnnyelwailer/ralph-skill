...
4. **Agents are untrusted.** The aloop runtime is the single trust boundary. Agents never call GH APIs, network endpoints, or the `gh` CLI directly. All external operations are mediated by the runtime — agents only express intent, the runtime decides what to execute.
