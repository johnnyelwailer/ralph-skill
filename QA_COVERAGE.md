...
| `aloop setup --non-interactive --mode loop` (packaged install) | 2026-03-16 | 5d985d8 | FAIL | Command rejects SPEC-documented mode flag: `error: unknown option '--mode'`. Tested iter 97. |
| `aloop gh watch --repo owner/repo` (packaged install) | 2026-03-16 | 5d985d8 | PASS | PATH-hardening regression fixed; no longer blocked by `gh: blocked by aloop PATH hardening`. Current failure is expected invalid-repo GraphQL error. |
| `aloop orchestrate --spec NONEXISTENT.md` (packaged install) | 2026-03-16 | 5d985d8 | PARTIAL | Exits 1, but still leaks raw JS stack frames from `dist/index.js` instead of clean validation-only output. Retested iter 97. |
| Dashboard layout @1920x1080 | 2026-03-16 | 5d985d8 | PASS | Screenshot captured and layout metrics pass (`panelGuess=6`, `sidebarVisible=true`, `docsVisible=true`, `activityVisible=true`). Evidence: `.../session-state/5b45498e-.../files/qa-20260316-150207/dashboard-1920x1080.png`. |
| `aloop scaffold` (fresh `HOME`, packaged install) | 2026-03-16 | ee79af0 | FAIL | Still crashes with `Template not found: .../.aloop/templates/PROMPT_plan.md` and leaks raw stack frames (`dist/index.js`). Evidence: `/home/pj/.copilot/session-state/1ec638f6-276a-40c8-8066-dad24123da7e/files/qa-20260316-154235/command-transcript.log`. |
| `aloop setup --non-interactive --mode loop` (fresh `HOME`, packaged install) | 2026-03-16 | ee79af0 | FAIL | Accepts `--mode` but fails during scaffold bootstrap with same missing-template error and stack trace leak. |
| `aloop devcontainer` (packaged install) | 2026-03-16 | ee79af0 | PASS | Successfully generated `.devcontainer/devcontainer.json` in isolated test project. |
| `aloop gh watch --repo octo-org/does-not-exist` (packaged install, unauthenticated) | 2026-03-16 | ee79af0 | PASS | No PATH-hardening block; fails cleanly with expected `gh auth login` guidance. |
| `aloop dashboard --port 4141` layout @1920x1080 (packaged install, isolated project) | 2026-03-16 | ee79af0 | FAIL | Returned fallback page `Dashboard assets not found at <project>/dashboard/dist`; Playwright check: `visiblePanels=0`, `sessions/docs/activity=false`. Screenshot: `/home/pj/.copilot/session-state/1ec638f6-276a-40c8-8066-dad24123da7e/files/qa-20260316-154235/dashboard-1920x1080.png`. |
