...
| `aloop start` (packaged install, loop mode, fresh `HOME`) | 2026-03-17 | 9ec9cb8 | FAIL | Regression: after successful setup, `aloop start --max-iterations 1` fails with `Loop script not found: <HOME>/.aloop/bin/loop.sh`. |
...
| Dashboard layout @1920x1080 (host dashboard URL from `meta.json`) | 2026-03-17 | 9ec9cb8 | FAIL | Playwright check still shows layout mismatch (`visibleAside=false`, `panelGuess=2`, `hasActivity=false`). Screenshot: `/home/pj/.copilot/session-state/aa6c290a-09a3-45eb-9eed-fc4a07df59a7/files/qa-iter174/dashboard-1920x1080-host.png`. |
