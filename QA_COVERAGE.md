...
| `aloop orchestrate --spec` (packaged install) | 2026-03-16 | 835c6fa | FAIL | Still leaks raw JS stack frames instead of clean error. Retested 835c6fa. |
...
| `aloop setup --non-interactive` (fresh `HOME`) | 2026-03-19 | current | PASS | Successfully bootstrapped templates and wrote config in isolated HOME. |
| `aloop start` (initialization) | 2026-03-19 | current | PASS | Correctly initializes session, worktree, and loop harness. |
| `aloop start --mode single` | 2026-03-19 | current | FAIL | Mode accepted by CLI, but `PROMPT_single.md` is missing from scaffolded prompts. |
| `aloop start` (stale runtime warning) | 2026-03-19 | current | FAIL | Warning compares against current directory's git repo instead of aloop source repo. |
| `Dashboard docs data (/api/state)` | 2026-03-19 | current | PASS | `docs` payload populated with `SPEC.md` content from worktree. |
| `aloop status --watch` | 2026-03-19 | current | PASS | Live terminal monitoring with 2s refresh works. |
| `aloop update` (permissions) | 2026-03-19 | current | PASS | Runtime assets refreshed and permissions set to 0o755. |
| `aloop devcontainer` (packaged install) | 2026-03-19 | current | PASS | Successfully generated `.devcontainer/devcontainer.json` with auth warnings. |
| `aloop orchestrate --plan-only` | 2026-03-19 | current | PASS | Initialized orchestrator session and state file correctly. |
| Dashboard layout/docs visibility @1920x1080 (host dashboard URL from `meta.json`) | 2026-03-19 | 1077741 | PASS | Playwright desktop check at `http://localhost:4040`: `sidebarVisible=true`, `hasDocuments=true`, `hasActivity=true`, `panelGuess=5`; screenshot + metrics at `.qa-tmp/qa-test-20260319-9bjfrk/evidence/dashboard-1920x1080.png` and `.qa-tmp/qa-test-20260319-9bjfrk/evidence/dashboard-layout.json`. |
| `aloop setup --non-interactive` (packaged install, isolated `HOME`) | 2026-03-19 | 1077741 | PASS | Happy path succeeds; invalid provider path returns clean validation error (`Unknown provider(s): badprovider`, exit 1). |
| `aloop start` + `aloop status` + `aloop stop` (packaged install, isolated `HOME`) | 2026-03-19 | 1077741 | PASS | `start --max-iterations` initializes loop, `status` lists session/provider health, `status --watch` refreshes every 2s, `stop` terminates session. |
| `aloop steer` (packaged install, isolated session) | 2026-03-19 | 1077741 | PASS | `steer --session <id> '<instruction>'` queues instruction successfully; ambiguity path without valid session id returns clean multi-session guidance. |
| `aloop orchestrate --spec` multi-file glob (`--plan-only`) | 2026-03-19 | 1077741 | PASS | `--spec 'SPEC.md specs/*.md' --plan-only` resolves all spec files and initializes orchestrator; nonexistent spec returns clean `No spec files found matching` error (exit 1). |
| `aloop start --mode single` (packaged install) | 2026-03-19 | 1077741 | FAIL | Still fails with `Error: Invalid mode: single`; existing `single mode parity gap` bug remains unresolved (re-test). |
