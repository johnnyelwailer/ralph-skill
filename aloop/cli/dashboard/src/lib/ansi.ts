  ...
  while ((m = re.exec(text)) !== null) {
    ...
    while (i < cmds.length) {
      ...
      if (isNaN(num) || num === 0) {
        ...
        const isFg = num === 38;
        const next = cmds[i + 1];
        if (next === '5' && cmds[i + 2] !== undefined) {
          const idx = parseInt(cmds[i + 2], 10);
          if (idx >= 0 && idx < 256) {
            const c = PALETTE_256[idx];
            if (isFg) style.fg = rgbStr(...c); else style.bg = rgbStr(...c);
          }
          i += 2;
        } else if (next === '2' && cmds[i + 2] !== undefined && cmds[i + 3] !== undefined && cmds[i + 4] !== undefined) {
          const r = parseInt(cmds[i + 2], 10), g = parseInt(cmds[i + 3], 10), b = parseInt(cmds[i + 4], 10);
          if (isFg) style.fg = rgbStr(r, g, b); else style.bg = rgbStr(r, g, b);
          i += 4;
        }
      }
