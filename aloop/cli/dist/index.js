...
function formatReviewCommentHistory(comments) {
  if (!Array.isArray(comments) || comments.length === 0)
    return "";
  const formatted = comments.filter((comment) => Boolean(comment?.body?.trim())).map((comment) => {
...
`;
            ...
            if (repo) {
              try {
                const commentsResult = spawnSync7("gh", ["pr", "view", String(prNumber), "--repo", repo, "--json", "comments"], { encoding: "utf8", stdio: ["pipe", "pipe", "pipe"] });
