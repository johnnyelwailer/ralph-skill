...
export interface QueueInvestigationRequest extends BaseRequest {
...
async function handleRequest(request: AgentRequest, fileName: string, options: RequestProcessorOptions): Promise<void> {
  switch (request.type) {
    ...
    case 'queue_investigation':
      return handleQueueInvestigation(request, fileName, options);
    default:
...
async function handleQueueInvestigation(request: QueueInvestigationRequest, fileName: string, options: RequestProcessorOptions): Promise<void> {
  const results: Array<{ id: string; issue_number?: number; status: string }> = [];
  for (const q of request.payload.questions) {
    const title = `[spec-question] ${q.question}`;
    const bodyParts = [
      `## Spec Question`,
      '',
      q.question,
    ...
    if (q.spec_section) {
      bodyParts.push(`**Spec Section:** ${q.spec_section}`, '');
    }
    if (q.code_path) {
      bodyParts.push(`**Code Path:** \`${q.code_path}\``, '');
    }
    if (q.risk) {
      bodyParts.push(`**Risk Level:** ${q.risk}`, '');
    }
    bodyParts.push(
      '---',
      '',
      '*This issue was generated by the aloop investigator agent. Please respond with the answer or resolution.*',
    );

    const body = bodyParts.join('\n');
    const bodyFile = path.join(options.aloopDir, 'requests', `_tmp_investigate_${request.id}_${q.id}.md`);
    await fs.writeFile(bodyFile, body, 'utf8');

    try {
      const tempRequestPath = path.join(options.aloopDir, 'requests', `_tmp_${request.id}_${q.id}.json`);
      await fs.writeFile(tempRequestPath, JSON.stringify({
        type: 'issue-create',
        title,
        body,
        labels: ['aloop/spec-question', 'aloop'],
      }));
      ...
      if (result.exitCode !== 0) {
        throw new Error(`issue-create failed for investigation ${q.id}: ${result.output}`);
      }
      const parsed = JSON.parse(result.output);
      results.push({ id: q.id, issue_number: parsed.number, status: 'created' });
    } catch (e) {
  ...
  await writeSuccessToQueue(request, { questions: results }, options, fileName);
}
