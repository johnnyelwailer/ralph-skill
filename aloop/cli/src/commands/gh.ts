...
import {
  ...
  GH_WATCH_DEFAULT_MAX_CONCURRENT,
  ...
  type GhWatchState,
  type GhWatchCycleSummary,
  type GhWatchIssue,
  type GhStartCommandOptions,
  type GhStartResult,
  type GhWatchCommandOptions,
} from '../lib/gh-state.js';
...
import {
  ...
  type PrCheckRun,
  type PrFeedback,
} from '../lib/gh-feedback-collect.js';
...
} from '../lib/gh-feedback-steer.js';

const execFileAsync = promisify(execFile);

// Late-bind startIssue to break the circular dependency with gh-state.ts
ghLoopRuntime.startIssue = (options) => ghStartCommandWithDeps(options);
...
function parsePrReference(raw: string): { number: number | null; url: string | null } {
  ...
  if (!trimmed) return { number: null, url: null };
  const match = trimmed.match(/\/pull\/(\d+)/);
  return { number: match ? Number.parseInt(match[1], 10) : null, url: trimmed };
}
...
function normalizeIssuePayload(payload: unknown, expectedIssueNumber: number): GhIssueView {
  ...
  if (!title || !url) throw new Error('Issue payload is missing required title/url fields.');
  ...
  const comments = Array.isArray(issue.comments)
    ...
    : [];
  return { number, title, body: typeof issue.body === 'string' ? issue.body : '', url, labels, comments };
}
...
function buildIssueContextBlock(issue: GhIssueView, specContent: string | null): string {
  ...
  if (commentLines.length > 0) parts.push('', '## Recent Comments', '', ...commentLines);
  if (specContent !== null) parts.push('', '## Additional Spec Context (--spec)', '', specContent.trim() || '(empty)');
...
export async function ghStartCommandWithDeps(options: GhStartCommandOptions, deps: GhStartDeps = defaultGhStartDeps): Promise<GhStartResult> {
  ...
  if (!issueNumber) throw new Error('gh start requires --issue <number>.');
  ...
  if (requestedRepo) issueViewArgs.push('--repo', requestedRepo);
  ...
  if (!issueRepo) warnings.push('Could not infer repository from issue URL; PR creation/link-back will require --repo.');
  ...
  if (typeof options.spec === 'string' && options.spec.trim()) {
    ...
    if (!deps.existsSync(specPath)) throw new Error(`--spec file not found: ${specPath}`);
  ...
  if (!deps.existsSync(planPromptPath)) throw new Error(`Missing planner prompt: ${planPromptPath}`);
  ...
  if (isTerminalState(completionState) && issueRepo) {
    ...
    const prCreate = await deps.execGh([
      'pr', 'create', '--repo', issueRepo, '--base', baseBranch, '--head', desiredBranch, '--title', prTitle, '--body', prBody,
    ]);
  ...
  const trackedEntry = watchEntryFromStartResult({
    issue: { number: issue.number, title: issue.title, url: issue.url, repo: issueRepo },
    session: {
...
function includesAloopTrackingLabel(targetLabels: unknown): boolean {
  ...
  if (typeof targetLabels === 'string') {
    return targetLabels.split(',').map((l) => l.trim()).some((l) => l === 'aloop' || l === 'aloop/auto');
  }
  return false;
}
  ...
  switch (operation) {
    case 'pr-create': {
      ...
      if (Array.isArray(payload.labels)) {
        for (const label of payload.labels) args.push('--label', String(label));
      }
      return args;
    }
    case 'pr-comment': {
    ...
    case 'issue-close':
      return ['issue', 'close', String(payload.issue_number), '--repo', repo];
    case 'issue-label': {
      const issueNum = enforced.issue_number ?? payload.issue_number;
      ...
      if (action === 'add') args.push('--add-label', String(label));
      else args.push('--remove-label', String(label));
      return args;
    }
    case 'pr-merge': {
...
function parseGhOutput(operation: string, stdout: string): Record<string, unknown> {
  ...
  if (operation === 'pr-create') {
    const match = trimmed.match(/\/pull\/(\d+)/);
    if (match) result.pr_number = parseInt(match[1], 10);
    if (trimmed) result.url = trimmed;
  } else if (operation === 'issue-create') {
    const match = trimmed.match(/\/issues\/(\d+)/);
    if (match) result.issue_number = parseInt(match[1], 10);
    if (trimmed) result.url = trimmed;
...
async function executeGhOperation(operation: string, options: any) {
  ...
  try {
    if (!fs.existsSync(configFile)) throw new Error(`Session config not found: ${configFile}`);
    ...
    const childCreatedPrNumbers = Array.isArray(config.created_pr_numbers)
      ? config.created_pr_numbers.map((v: unknown) => parsePositiveInteger(v)).filter((v: number | undefined): v is number => v !== undefined)
      : [];
    sessionPolicy = { repo: config.repo, assignedIssueNumber, childCreatedPrNumbers };
  } catch (e: any) {
    const timestamp = new Date().toISOString();
    const logEntry = { timestamp, event: 'gh_operation_denied', type: operation, session: options.session, role, reason: e.message };
  ...
  if (needsRequestFile) {
    if (typeof requestFile !== 'string' || !requestFile.trim()) { console.error(`Request file not provided for operation: ${operation}`); process.exit(1); }
    if (fs.existsSync(requestFile)) {
      try { requestPayload = JSON.parse(fs.readFileSync(requestFile, 'utf8')); } catch (e) { console.error(`Failed to parse request file: ${requestFile}`); process.exit(1); }
    } else { console.error(`Request file not found: ${requestFile}`); process.exit(1); }
  } else {
    requestPayload = { since: options.since };
  }
  ...
  if (!allowed) {
    const logEntry = { timestamp, event: 'gh_operation_denied', type: operation, session: options.session, role, reason: reason || `${operation} not allowed for ${role} role` };
    appendLog(sessionDir, logEntry);
    console.error(JSON.stringify(logEntry));
    process.exit(1);
  } else {
    ...
    try {
      ...
      const errorEntry = { timestamp, event: 'gh_operation_error', type: operation, session: options.session, role, request_file: requestFileName, error: e.message, stderr: e.stderr || '', enforced };
      appendLog(sessionDir, errorEntry);
      console.error(JSON.stringify(errorEntry));
      process.exit(1);
    }
    let parsed: Record<string, unknown> = {};
    try { parsed = parseGhOutput(operation, ghResult!.stdout); } catch (e: any) {
      const parseErrorEntry: any = { timestamp, event: 'gh_operation_error', type: operation, session: options.session, role, error: e.message, stderr: ghResult!.stderr || '', enforced };
      if (requestFileName) parseErrorEntry.request_file = requestFileName;
      appendLog(sessionDir, parseErrorEntry);
      console.error(JSON.stringify(parseErrorEntry));
      process.exit(1);
    }
    const logEntry: any = { timestamp, event: 'gh_operation', type: operation, session: options.session, role, result: 'success', enforced, ...parsed };
    if (requestFileName) logEntry.request_file = requestFileName;
    appendLog(sessionDir, logEntry);
...
function evaluatePolicy(
  ...
  if (payload.repo && payload.repo !== sessionPolicy.repo) {
    return { allowed: false, reason: `Mismatched repo: requested ${payload.repo}, but session is bound to ${sessionPolicy.repo}` };
  }
  ...
  if (role === 'child-loop') {
    switch (operation) {
      ...
      case 'issue-comment': {
        ...
        if (targetIssueNumber === undefined) return { allowed: false, reason: 'Child issue-comment requires numeric issue_number' };
        if (sessionPolicy.assignedIssueNumber === undefined) return { allowed: false, reason: 'Child session is missing assigned issue scope in config' };
        if (targetIssueNumber !== sessionPolicy.assignedIssueNumber) {
          return { allowed: false, reason: `Child issue-comment must target assigned issue #${sessionPolicy.assignedIssueNumber}` };
        }
      ...
      case 'pr-comment': {
        ...
        if (targetPrNumber === undefined) return { allowed: false, reason: 'Child pr-comment requires numeric pr_number' };
        if (!sessionPolicy.childCreatedPrNumbers.includes(targetPrNumber)) {
          return { allowed: false, reason: `Child pr-comment must target a PR created by this session (${targetPrNumber} is out of scope)` };
        }
    ...
    switch (operation) {
      case 'issue-create':
        if (!includesAloopTrackingLabel(payload.labels)) return { allowed: false, reason: 'Must include aloop tracking label' };
        return { allowed: true, enforced: { repo: sessionPolicy.repo } };
      case 'issue-close':
        if (!includesAloopTrackingLabel(payload.target_labels)) return { allowed: false, reason: 'issue-close requires aloop-scoped target validation' };
        return { allowed: true, enforced: { repo: sessionPolicy.repo } };
      case 'pr-create':
        return { allowed: true, enforced: { base: 'agent/trunk', repo: sessionPolicy.repo } };
      case 'pr-merge':
      ...
      case 'issue-label': {
        if (!includesAloopTrackingLabel(payload.target_labels)) return { allowed: false, reason: 'issue-label requires aloop-scoped target validation' };
        const issueNumber = parsePositiveInteger(payload.issue_number);
        if (issueNumber === undefined) return { allowed: false, reason: 'issue-label requires numeric issue_number' };
        const action = payload.label_action;
        if (action !== 'add' && action !== 'remove') return { allowed: false, reason: 'issue-label requires label_action: add or remove' };
        if (payload.label !== 'aloop/blocked-on-human') return { allowed: false, reason: 'issue-label only permits aloop/blocked-on-human' };
        return { allowed: true, enforced: { repo: sessionPolicy.repo, issue_number: issueNumber, label_action: action, label: 'aloop/blocked-on-human' } };
      }
      case 'issue-comments':
      case 'pr-comments':
        if (typeof payload.since !== 'string' || !payload.since.trim()) return { allowed: false, reason: `${operation} requires --since timestamp` };
        return { allowed: true, enforced: { repo: sessionPolicy.repo, since: payload.since.trim() } };
      case 'pr-comment':
      case 'issue-comment':
        if (!includesAloopTrackingLabel(payload.target_labels)) return { allowed: false, reason: `${operation} requires aloop-scoped target validation` };
        return { allowed: true, enforced: { repo: sessionPolicy.repo } };
      case 'branch-delete':
...
async function fetchMatchingIssues(options: GhWatchCommandOptions): Promise<GhWatchIssue[]> {
  ...
  for (const label of labels) { if (label.trim()) args.push('--label', label.trim()); }
  if (typeof options.assignee === 'string' && options.assignee.trim()) args.push('--assignee', options.assignee.trim());
  if (typeof options.milestone === 'string' && options.milestone.trim()) args.push('--milestone', options.milestone.trim());
  if (typeof options.repo === 'string' && options.repo.trim()) args.push('--repo', options.repo.trim());
  try {
...
async function launchTrackedIssue(issueNumber: number, options: GhWatchCommandOptions, state: GhWatchState): Promise<GhWatchIssueEntry> {
  const result = await ghLoopRuntime.startIssue({
    issue: issueNumber, repo: options.repo, homeDir: options.homeDir, projectRoot: options.projectRoot, provider: options.provider, max: options.max, output: 'json',
  });
...
export async function finalizeWatchEntry(entry: GhWatchIssueEntry, options: GhWatchCommandOptions): Promise<boolean> {
  if (!entry.repo || !entry.branch || !entry.session_id || !entry.completion_state) return false;
  ...
  if (fs.existsSync(metaPath)) {
    try {
      ...
      if (typeof meta.project_root === 'string' && meta.project_root.trim()) projectRoot = meta.project_root;
    } catch { /* ignore */ }
  }
  ...
  try {
    ...
    if (issuePayload.title) issueTitle = issuePayload.title;
  } catch { /* ignore */ }
  let baseBranch: 'agent/main' | 'main' = 'main';
  try {
    ...
    try { await execFileAsync('git', ['-C', projectRoot, 'branch', 'agent/main', 'main']); baseBranch = 'agent/main'; } catch { baseBranch = 'main'; }
  }
  ...
  try {
    const prCreate = await ghExecutor.exec(['pr', 'create', '--repo', entry.repo, '--base', baseBranch, '--head', entry.branch, '--title', prTitle, '--body', prBody]);
    ...
    try {
      const prList = await ghExecutor.exec(['pr', 'list', '--repo', entry.repo, '--head', entry.branch, '--json', 'number,url']);
      const existing = JSON.parse(prList.stdout) as { number: number; url: string }[];
      if (existing.length > 0) { entry.pr_number = existing[0].number; entry.pr_url = existing[0].url; }
    } catch { /* ignore */ }
  }
  if (entry.pr_number === null) return false;
  const summary = [
...
export async function runGhWatchCycle(options: GhWatchCommandOptions): Promise<GhWatchCycleSummary> {
  ...
  for (const issue of matchedIssues) {
    if (!state.issues[String(issue.number)]) enqueueIssue(state, issue);
  }

  const feedbackResumed: number[] = [];
  const completedWithPr = Object.values(state.issues).filter((e) => e.status === 'completed' && e.pr_number !== null);
  for (const entry of completedWithPr) {
    const resumed = await checkAndApplyPrFeedback(entry, options);
    if (resumed) feedbackResumed.push(entry.issue_number);
  }

  const newlyCompleted = Object.values(state.issues).filter(
    (e) => (e.status === 'completed' || e.status === 'stopped') && isTerminalState(e.completion_state) && !e.completion_finalized
  );
  for (const entry of newlyCompleted) {
    const success = await finalizeWatchEntry(entry, options);
    if (success) { entry.completion_finalized = true; entry.updated_at = ghLoopRuntime.now(); }
  }
  ...
  while (running < maxConcurrent && state.queue.length > 0) {
    ...
    if (launched.status === 'running') running += 1;
  }

  saveWatchState(options.homeDir, state);
  return { started, queued, active: running, tracked: Object.keys(state.issues).length, feedback_resumed: feedbackResumed };
}

export async function ghWatchCommand(options: GhWatchCommandOptions): Promise<void> {
  ...
  if (runOnce) {
    ...
    if (outputMode === 'json') { console.log(JSON.stringify(summary, null, 2)); return; }
  ...
  let stopping = false;
  const markStopping = (): void => { stopping = true; };
  process.on('SIGINT', markStopping);
  process.on('SIGTERM', markStopping);
  try {
    while (!stopping) {
      const summary = await runGhWatchCycle(options);
      if (outputMode === 'json') { console.log(JSON.stringify(summary)); }
      else { console.log(`watch cycle complete: started=${summary.started.length} queued=${summary.queued.length} active=${summary.active} tracked=${summary.tracked} feedback_resumed=${summary.feedback_resumed.length}`); }
      if (stopping) break;
...
function failGhWatch(outputMode: GhOutputMode, error: unknown): never {
  ...
  if (outputMode === 'json') { console.log(JSON.stringify({ success: false, error: message })); }
  else { console.error(message); }
  return process.exit(1) as never;
}

export function formatGhStatusRows(state: GhWatchState, sessionsById: Map<string, { session_id: string; iteration?: number | null }>): string {
  const entries = Object.values(state.issues).sort((a, b) => a.issue_number - b.issue_number);
  if (entries.length === 0) return 'No GH-linked sessions.';
  const lines: string[] = ['Issue  Branch                PR    Status      Iteration  Feedback'];
...
export async function ghStatusCommand(options: GhStatusCommandOptions): Promise<void> {
  const outputMode = options.output ?? 'text';
  const state = loadWatchState(options.homeDir);
  const sessionsById = await refreshWatchState(options.homeDir, state);
  saveWatchState(options.homeDir, state);
  const entries = Object.values(state.issues).sort((a, b) => a.issue_number - b.issue_number);
  if (outputMode === 'json') { console.log(JSON.stringify({ issues: entries }, null, 2)); return; }
  console.log(formatGhStatusRows(state, sessionsById));
}

export async function ghStopCommand(options: GhStopCommandOptions): Promise<void> {
  const outputMode = options.output ?? 'text';
  const issueNumber = parsePositiveInteger(options.issue);
  const stopAll = options.all === true;
  if (!stopAll && !issueNumber) throw new Error('gh stop requires either --issue <number> or --all.');
  if (stopAll && issueNumber) throw new Error('gh stop accepts either --issue or --all, not both.');
  const state = loadWatchState(options.homeDir);
  await refreshWatchState(options.homeDir, state);
  const targets = stopAll
    ? Object.values(state.issues)
    : issueNumber ? [state.issues[String(issueNumber)]].filter((e): e is GhWatchIssueEntry => Boolean(e)) : [];
  if (!stopAll && issueNumber && targets.length === 0) throw new Error(`No GH-linked session found for issue #${issueNumber}.`);
  ...
  if (outputMode === 'json') { console.log(JSON.stringify({ stopped: results, failed: failed.length }, null, 2)); }
  else if (results.length === 0) { console.log('No GH-linked sessions to stop.'); }
  else {
    for (const result of results) {
      if (result.success) console.log(`Stopped GH-linked issue #${result.issue_number}${result.session_id ? ` (${result.session_id})` : ''}.`);
      else console.log(`Failed to stop issue #${result.issue_number}: ${result.reason ?? 'unknown error'}`);
    }
  }
  if (failed.length > 0) process.exit(1);
}

// Commander subcommand registrations

function addGhRequestSubcommand(name: string, description: string) {
  return ghCommand.command(name).description(description)
    ...
    .action(withErrorHandling(async (options) => { await executeGhOperation(name, options); }));
}
...
ghCommand.command('start').description('Start a GitHub-linked aloop session for an issue')
  ...
  .action(withErrorHandling(async (options: GhStartCommandOptions) => {
    ...
    if (options.output === 'json') { console.log(JSON.stringify(result, null, 2)); return; }
...
ghCommand.command('watch').description('Monitor matching issues and start GH-linked loops with queueing')
  ...
  .option('--dry-run', 'Print what would be started/queued and exit without launching')
  .option('--max-ci-retries <n>', 'Max consecutive identical CI failures before halting re-iteration', '3')
  .option('--output <mode>', 'Output format: json or text', 'text')
  .action(async (options: GhWatchCommandOptions) => {
    ...
    try {
      if (options.dryRun) {
        const state = loadWatchState(options.homeDir);
        const matchedIssues = await fetchMatchingIssues(options);
        const wouldStart: number[] = [];
        const wouldQueue: number[] = [];
        for (const issue of matchedIssues) {
          if (!state.issues[String(issue.number)]) wouldQueue.push(issue.number);
        }
        if (outputMode === 'json') {
          console.log(JSON.stringify({ dry_run: true, would_start: wouldStart, would_queue: wouldQueue, sessions_started: 0 }, null, 2));
        } else {
          console.log(`dry-run: issues matching filter: ${wouldQueue.join(', ') || 'none'}`);
          console.log(`dry-run: sessions_started=0`);
        }
        return;
      }
...
ghCommand.command('status').description('Show GH-linked issue/session/PR state from watch tracking')
  .option('--home-dir <path>', 'Home directory override')
  .option('--output <mode>', 'Output format: json or text', 'text')
  .action(withErrorHandling(async (options: GhStatusCommandOptions) => { await ghStatusCommand(options); }));

ghCommand.command('stop').description('Stop GH-linked loops for one issue or all tracked issues')
  .option('--issue <number>', 'GitHub issue number to stop')
  .option('--all', 'Stop all tracked GH-linked loops')
  .option('--home-dir <path>', 'Home directory override')
  .option('--output <mode>', 'Output format: json or text', 'text')
  .action(withErrorHandling(async (options: GhStopCommandOptions) => { await ghStopCommand(options); }));
...
// Re-exports for backward compatibility with existing tests and consumers
export {
  ghExecutor,
  selectUsableGhBinary,
  ghLoopRuntime,
...
export type { PrReviewComment, PrIssueComment, PrCheckRun, PrFeedback, GhWatchIssueEntry, GhWatchIssueStatus };
