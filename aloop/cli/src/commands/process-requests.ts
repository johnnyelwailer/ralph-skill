...
export async function processRequestsCommand(options: ProcessRequestsOptions): Promise<void> {
  ...
  if (repo && stateChanged && adapter?.setIssueStatus) {
    let synced = 0;
    for (const issue of state.issues) {
      if (!issue.number || !issue.status) continue;
      try {
        const currentStatus = await adapter.getIssueStatus?.(issue.number);
        if (currentStatus?.toLowerCase() !== issue.status.toLowerCase()) {
          await adapter.setIssueStatus(issue.number, issue.status);
          synced++;
        }
