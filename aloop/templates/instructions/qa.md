...
0d. **Check coverage gaps**: Read @QA_COVERAGE.md.
   - **If missing**: Create it. Initialize it by reading {{SPEC_FILES}} and extracting all top-level features/capabilities. Mark all as `UNTESTED`.
   - **Baseline**: The baseline features should come from the "Architecture" or "Features" sections of the spec. Each feature becomes a row in the coverage table.
   - **Use coverage gaps to find features marked "never" tested or previously "FAIL"**.
...
5. **Update QA Coverage**
   Update @QA_COVERAGE.md with test results. Use the following 7-column format (pipe-delimited):
   ```
