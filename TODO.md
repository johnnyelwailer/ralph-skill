...
- [x] [qa/P1] Dashboard health tab missing `codex` when no recent codex event exists — fixed by teaching `deriveProviderHealth` to accept an optional `configuredProviders` baseline from `meta.json` (`enabled_providers` / `round_robin_order`). Configured providers without log events now appear with `unknown` status and "no activity" label. Added `HealthPanel` UI for unknown status (grey circle). 7 new tests in `App.test.tsx`. (priority: low) — **COMPLETED** (iter 290)
