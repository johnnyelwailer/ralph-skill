...
- [x] [review] Gate 2: add error-path tests for `stop` kill failures/permission errors and malformed session/health JSON handling in `aloop/cli/aloop.mjs.test.mjs` and `aloop/cli/lib/session.mjs` tests so broken behavior fails tests. (priority: high)
