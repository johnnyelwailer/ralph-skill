...
#### High Priority
- [x] [orchestrator/P0] Orchestrator must be fully autonomous — DONE: spawns loop.sh, registers active.json, process-requests called between iterations. Remaining issues below.
- [ ] [orchestrator/P1] Orchestrator agent file writes — decompose agent produces text output but doesn't write results JSON. Needs stronger prompt instructions and verification that the provider used (claude vs opencode) supports file writes in --print mode. May need provider pinning for orchestrator prompts. (priority: high)
- [ ] [orchestrator/P1] Orchestrator provider routing — decompose prompt used opencode/gemini despite --provider claude --round-robin claude. Frontmatter provider field in queue prompts may override CLI args. Investigate and fix. (priority: high) — currently runs synchronously and exits after init. Must spawn background daemon (like `aloop start` does), register in `active.json`, run scan loop indefinitely, be visible in dashboard, stoppable via `aloop stop`. See SPEC-ADDENDUM "Orchestrator Must Be Fully Autonomous". (priority: critical)
...
- [ ] [dashboard/P1] Regression: sidebar session grouping broken — sessions no longer grouped by project, showing flat list instead of grouped tree. (priority: high)
- [ ] [dashboard/P1] Regression: log entries missing agent output and artifacts
- [ ] [dashboard/P1] Regression: health tab shows "No provider data yet" even when provider health data exists — previously showed provider status with colored indicators — iteration entries no longer show the agent's output text or linked artifacts (screenshots, proof files). Previously worked with expandable iteration details. (priority: high)
...
- [ ] [setup/P1] Spec consistency pass — setup skill should run a coherence check across all spec files (SPEC.md + SPEC-ADDENDUM.md + any addenda), detect contradictions, stale cross-references, conflicting acceptance criteria, and duplicated/divergent requirements. Auto-fix obvious issues (stale refs, duplicated text, outdated names). Only surface ambiguous contradictions to the user. (priority: high)
...
- [x] [qa/P1] Dashboard health tab missing `codex` when no recent codex event exists — fixed by teaching `deriveProviderHealth` to accept an optional `configuredProviders` baseline from `meta.json` (`enabled_providers` / `round_robin_order`). Configured providers without log events now appear with `unknown` status and "no activity" label. Added `HealthPanel` UI for unknown status (grey circle). 7 new tests in `App.test.tsx`. (priority: low) — **COMPLETED** (iter 290) [reviewed: gates 1-9 pass — iter 306]
- [x] [dashboard/low] Extend E2E `smoke.spec.ts` coverage for explicit 1920x1080 sidebar/docs/activity visibility checks once core gates are green. — **COMPLETED** (iter 291; strengthened desktop smoke assertions for sidebar + documents + activity panel visibility and column positioning at 1920x1080)
