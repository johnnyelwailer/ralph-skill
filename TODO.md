...
- [x] [qa/P2] `blockers.json` initialized as `{}` (empty object) causes fatal: TypeError: existingRecords.map is not a function. Repro: create blockers.json containing `{}` then run process-requests. Fix: after `JSON.parse`, validate result is an array; fall back to `[]` if not. (priority: medium)
