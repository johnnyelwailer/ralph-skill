...
- [x] [orchestrator/P1] Orchestrator agent file writes — fixed: cwd set to sessions/ parent so providers can write to session dir. Decomposition results now consumed by process-requests.
- [x] [orchestrator/P1] Orchestrator provider routing — fixed: removed hardcoded provider:claude, use worktree for proper project access so any provider works.
...
- [x] [cli/P1] Orchestrator not visible in dashboard — fixed: orchestrate now registers in active.json.
- [x] [dashboard/P1] Regression: sidebar session grouping broken — fixed: fallback to project_root basename when project_name missing
- [x] [dashboard/P1] Regression: log entries missing agent output and artifacts — fixed: show all structured log entries, not just 10 significant types
- [x] [dashboard/P1] Regression: health tab shows "No provider data yet" — fixed: add meta field to DashboardState for configured providers baseline

- [ ] [dashboard/P1] Orchestrator logs missing agent CLI output — orchestrator session activity shows structured events but not the actual agent output text (what the LLM wrote). Normal loop sessions show expandable iteration output. Orchestrator should be on par — show the agent's response text, tool calls, and file writes per iteration. (priority: high)
- [ ] [dashboard/P1] Orchestrator artifacts not viewable — epics, estimation results, decomposition plans etc. generated by orchestrator agents should be viewable in the dashboard. These are stored as JSON in requests/ and as markdown in queue/. Show them in the Documents tab or a dedicated Artifacts tab. (priority: high)
- [ ] [dashboard/P2] Stale sessions showing as active — completed sessions still appear in the "active" group in the sidebar. Dashboard should check PID liveness and move completed/dead sessions to the "recent" group. (priority: medium)
- [ ] [dashboard/P2] GitHub issues in sidebar tree — once orchestrator creates GitHub issues, show them in the sidebar under the orchestrator session as a tree: session → epics → sub-issues. Each issue should have a clickable link to the GitHub issue page. Status indicators (open/closed/in-progress) per issue. (priority: medium)
- [ ] [dashboard/P2] GitHub issue links — all references to issue numbers in log entries, orchestrator state, and activity should be clickable links to the GitHub issue (respecting GHE — derive URL from repo remote, never hardcode github.com). (priority: medium)

- [ ] [orchestrator/P1] Scan agent self-healing and alerting — scan agent detects blockers but can't fix them or notify anyone. Needs: (1) stuck detection — if same blocker persists N iterations, escalate, (2) structured diagnostics.json in session dir for dashboard display, (3) self-heal known issues (missing labels → create, missing config → derive from meta.json, unprocessed requests → log why), (4) user alerting via STEERING.md or dashboard notification banner. (priority: high)
- [ ] [orchestrator/P1] process-requests must handle all request types — currently only handles decomposition results and estimates. Needs to handle create_issues, update_issue, dispatch_child, merge_pr, and other request types that agents write. The scan agent writes valid requests but nobody processes them. (priority: high)
