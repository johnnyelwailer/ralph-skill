...
- [ ] [qa/P1] Additional components missing .test.tsx files: `session/ActivityPanel.tsx`, `session/ArtifactComparisonHeader.tsx`, `session/DiffOverlayView.tsx`, `session/SideBySideView.tsx` — ran `find components/ -name '*.tsx' | grep -v test | grep -v stories | grep -v /ui/` and diff'd against existing .test.tsx files → 4 components have no test file at all → SPEC-ADDENDUM AC: "Every component in `components/` has a corresponding `.test.tsx` file." Create unit test files for each. Tested at iter 1. (priority: high)
- [ ] [qa/P1] Additional components missing .stories.tsx files: `layout/ResponsiveLayout.tsx`, `session/ActivityPanel.tsx`, `session/ArtifactComparisonDialog.tsx`, `session/ArtifactComparisonHeader.tsx`, `session/DiffOverlayView.tsx`, `session/ImageLightbox.tsx`, `session/LogEntryExpandedDetails.tsx`, `session/LogEntryRow.tsx`, `session/SideBySideView.tsx`, `session/SliderView.tsx` — ran find vs stories list → 10 components lack .stories.tsx → SPEC-ADDENDUM AC: "Every component in `components/` has a corresponding `.stories.tsx` file." Create stories with ≥2 stories each. Tested at iter 1. (priority: high)
