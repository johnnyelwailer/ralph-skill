...
- [ ] **Migrate orchestrate.ts — checkPrGates** — `checkPrGates` (lines 3518-3598) still calls `deps.execGh` directly for two PR view calls: (1) `--json mergeable,mergeStateStatus` and (2) `--json statusCheckRollup`. Replace with `deps.adapter.getPRStatus()` and `deps.adapter.getPrChecks()` (both already exist on GitHubAdapter). Use the same adapter-with-fallback pattern as `mergePr`. Note: `mergePr` and `flagForHuman` are already migrated — only `checkPrGates` remains in the PR lifecycle group.
...
- [ ] **Add adapter-path tests for checkPrGates** — Once checkPrGates is migrated, add tests injecting a mock adapter (no `execGh`) that verify: (1) `getPRStatus` is called with the correct PR number, (2) `getPrChecks` is called, (3) results map correctly to gate pass/fail/pending. Follow the `mergePr adapter path` describe block at `orchestrate.test.ts:6721` as the pattern.
