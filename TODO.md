...
- [ ] [review] Gate 3: `QACoverageBadge.tsx` still has 2 untested branches — new-module ≥90% threshold not fully met. Add to `QACoverageBadge.test.tsx`: (1) `!response.ok` early return (line 69) — render with `mockFetch(data, false)` (pass `ok=false` second arg) and assert the component renders `null` (no button rendered after waiting); (2) `sessionId={null}` → sp='' path (line 67) — render `<QACoverageBadge sessionId={null} refreshKey="k1" />` with a `vi.spyOn(global, 'fetch')` and assert the fetch URL argument contains no `?session=` substring. (priority: high)
