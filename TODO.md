...
- [ ] [review] Gate 1: `handleMergePr` (requests.ts:642-644) writes temp request file with only `type` and `pr_number` — `payload.strategy` is omitted. Downstream gh.ts hardcodes `--squash`. Fix: include `strategy: request.payload.strategy` in temp file JSON and update gh.ts `pr-merge` case to read the strategy field instead of hardcoding `--squash`. Add a test that asserts strategy is passed through. (priority: high)
- [ ] [review] Gate 4: `useIsTouchLikePointer` hook (dashboard/src/hooks/useIsTouchLikePointer.ts) and its test file are dead code — added but never imported anywhere in the dashboard. Either wire it into a component or remove both files. (priority: medium)
