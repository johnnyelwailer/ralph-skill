...
- [ ] [review] Gate 6: proof manifest skipped all artifacts, but this iteration changed observable CLI behavior (`aloop setup --non-interactive --mode orchestrate` now writes orchestrate mode/prompts). Add human-verifiable proof (CLI capture or before/after artifact diff) for that behavior. (priority: high)
