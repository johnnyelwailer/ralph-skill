...
spec-review (2026-03-30): Gate 9 README.md fix PASS. (1) README.md:130 now correctly states `concurrency_cap` is under the `loop:` section — matches `.aloop/pipeline.yml:56`. (2) YAML example updated to `concurrency_cap: 3` nested under `loop:` — value and nesting match canonical config. (3) Line 137 simplified correctly. (4) SPEC.md:3654-3669 confirms `concurrency_cap` absent from `loopSettings` (orchestrator-only), consistent with prior fix. All in-scope requirements met. No new gaps found.
