...
### Up Next
- [ ] [qa/P1] [bug] `single` mode parity gap: loop.sh (`resolve_iteration_mode` line 366) and loop.ps1 (`Resolve-IterationMode` line 260) accept and handle `single` mode, but `start.ts:20` (`LOOP_MODE_SET`) does NOT include `single`, so `aloop start --mode single` rejects it before ever reaching the loop scripts. Either add `'single'` to `LOOP_MODE_SET` and the `LoopMode` type, or remove `single` from both loop scripts' validation sets. QA tested at iter 251 — still failing. (priority: high)
