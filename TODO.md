...
- [ ] [qa/P1] [bug] `single` mode accepted by loop.sh/loop.ps1 validation but has no handling logic — `resolve_iteration_mode` has no case for `single`, CLI (`aloop setup`/`aloop start`) rejects it as invalid. Either implement the mode or remove it from the loop script validation sets. Tested at commit 0a3db40. (priority: high)
