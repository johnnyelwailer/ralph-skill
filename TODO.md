...
- [ ] [qa/P1] Devcontainer generation and verify flows must resolve providers from project config (`enabled_providers` / resolved runtime provider set), not `discoverWorkspace.providers.installed`. Current code in `devcontainer.ts` uses locally installed CLIs, which drops configured providers (especially `opencode`) from `postCreateCommand`, `remoteEnv`, and verify checks. Add regression tests covering configured-but-not-locally-installed providers. (priority: high)
...
- [ ] [qa/P1] Devcontainer auth preflight warnings for activated providers are missing. Implement host-side warnings when required auth env vars are absent (with Claude preference: `CLAUDE_CODE_OAUTH_TOKEN` then `ANTHROPIC_API_KEY`) so setup fails early with actionable guidance instead of deferred container-time failures. (priority: high)
