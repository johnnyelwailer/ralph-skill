...
### In Progress
- [ ] [review] Gate 3: Branch coverage evidence missing/failing for `aloop/bin/loop.sh` and `aloop/bin/loop.ps1`. The test harnesses (`loop.tests.ps1` and `loop_branch_coverage.tests.sh`) crash or fail to complete (e.g., `derive_mode_from_prompt_name: command not found`, `printf: - : invalid option`), preventing valid coverage artifacts from being generated. Fix the test suites so they generate passing >=80% coverage evidence. (priority: high)
- [ ] [review] Gate 5: Massive test failures (30+ in `loop.tests.ps1` and multiple in shell tests) because the new phase prerequisite rule (missing `TODO.md` forces plan) breaks all tests that verify 'build' mode transitions without first creating a `TODO.md` with an unchecked task. Update these tests to mock/create `TODO.md` appropriately. (priority: high)
- [ ] [review] Gate 9: `README.md` documents `aloop orchestrate --spec SPEC.md` but was not updated to document the new multi-file spec and glob support (e.g., `aloop orchestrate --spec "SPEC.md specs/*.md"`). Update README usage examples to reflect the new feature. (priority: medium)
- [ ] [review] Gate 4: `aloop start` crashes in packaged install with `deps.discoverWorkspace is not a function`. This is due to `commander` passing the `Command` object as the 3rd argument to `startCommand`, which is typed as `deps`. Implement defensive dependency resolution (like `resolveStartDeps`) in `start.ts` to fallback to `defaultDeps` when called by `commander`. (priority: high)
- [ ] [review] Gate 7: Dashboard desktop layout mismatch at 1920x1080 persists as a regression in the host dashboard (`visibleAside=false`, no visible sessions/docs/activity tokens). Verify breakpoints and panel visibility in the rendered DOM. (priority: high)
