...
### Up Next
- [x] [review] Gate 2: `LogEntryRow.test.tsx:147-165` — `triggers onComparison callback and shows comparison when callback invoked` — final assertion `expect(screen.getByTestId('log-entry-expanded-details')).toBeInTheDocument()` was already true before the comparison click; the mock only renders `data-testid="close-comparison"` when `showComparison` is truthy — add assertion `expect(screen.getByTestId('close-comparison')).toBeInTheDocument()` after clicking `trigger-comparison` to actually prove `onComparison` updated state (priority: high) [reviewed: gates 1-9 pass]
- [x] [review] Gate 2: `LogEntryRow.test.tsx:167-186` — `triggers onCloseComparison callback when close button clicked` — `fireEvent.click(closeComparison)` fires but no assertion follows; a broken `onCloseComparison` that never clears state would still pass — add `expect(screen.queryByTestId('close-comparison')).not.toBeInTheDocument()` after the click to verify state was cleared (priority: high) [reviewed: gates 1-9 pass]
- [x] [qa/P1] ResponsiveLayout.tsx branch coverage 75% (lines 45,92): add tests covering (a) `setSidebarOpen(value)` called on non-desktop breakpoint (exercises line 45 `setSidebarOpenState(next)`) and (b) calling `useResponsiveLayout()` outside `<ResponsiveLayout>` provider (exercises line 92 `throw new Error(...)`) — verify ≥90% branch coverage (priority: high) [reviewed: gates 1-9 pass]
