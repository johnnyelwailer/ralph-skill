...
### In Progress
- [x] [review] Gate 1: `aloop/bin/loop.sh` and `aloop/bin/loop.ps1` phase-prerequisite guards do not enforce `build -> plan` when `TODO.md` is missing/unreadable. SPEC requires build phase prerequisite to fail unless TODO exists with at least one unchecked task. Update both runtimes to force `plan` (and log `phase_prerequisite_miss` reason `no_tasks`) when the plan file is absent as well as when it has zero unchecked tasks. (priority: high)
- [x] [review] Gate 2: `aloop/cli/src/commands/project.test.ts` opencode-agent copy test uses non-specific assertions (`existsSync` + `content.length > 0`), which can pass with corrupted/wrong files. Replace with exact source-to-destination content assertions per agent file (`vision-reviewer.md`, `error-analyst.md`, `code-critic.md`) and add a negative assertion proving wrong content fails. (priority: high)
- [x] [review] Gate 3: touched file `aloop/cli/src/commands/start.ts` is below branch threshold. Current measured branch coverage is **70.68%** (`npx --yes tsx --test --experimental-test-coverage src/commands/start.test.ts`). Add branch-targeted tests for uncovered mode/validation/error branches and raise `start.ts` to >=80% branch coverage. (priority: high)
- [x] [review] Gate 3: no current branch-coverage evidence demonstrates >=80% for newly added phase-prerequisite branches in `aloop/bin/loop.sh` and `aloop/bin/loop.ps1` (new `check_phase_prerequisites` / `Check-PhasePrerequisites`, `check_has_builds_to_review` / `Check-HasBuildsToReview`, `lastPlanCommit` persistence paths). Add coverage artifacts that explicitly include these new branches and meet threshold. (priority: high)
- [x] [review] Gate 4: remove committed debug/repro artifacts `check_argv.mjs` and `reproduce_setup_issue.sh` (added in commit `54372ac`) or wire them into supported tooling/docs; they are currently unreferenced dead artifacts in repository root. (priority: medium)
- [x] [review] Gate 2: `aloop/cli/src/commands/project.test.ts:106-107` uses existence-only assertions (`existsSync`) for orchestrate prompt generation; strengthen this test to assert concrete generated prompt content (or exact prompt set) so it fails on wrong/partial copies, not just missing files. (priority: high)
- [x] [review] Gate 3: branch-coverage evidence is missing for touched code in `aloop/cli/lib/project.mjs` and `aloop/cli/src/commands/setup.ts` (new `resolvePromptTemplates`/orchestrate scaffold path and `mapSetupModeToLoopMode` orchestrate branch). Add branch-focused tests and provide per-file branch coverage output showing gate thresholds are met. (priority: high)
- [x] [review] Gate 6: proof manifest skipped all artifacts, but this iteration changed observable CLI behavior (`aloop setup --non-interactive --mode orchestrate` now writes orchestrate mode/prompts). Add human-verifiable proof (CLI capture or before/after artifact diff) for that behavior. (priority: high)
### Up Next — P1 Hardening
- [ ] [qa/P1] `aloop start` crashes in packaged install with dependency-injection runtime error (`deps.discoverWorkspace is not a function`): In isolated packaged install (`/tmp/aloop-test-install-OQybYO/bin/aloop`), running `aloop start --max-iterations 1` in both configured and non-configured repos exits 1 immediately with this error, so session setup/launch never begins. Spec requires `aloop start` to handle full session setup + launch path with user-facing outcomes (dashboard URL/start summary or clean validation errors), not internal DI crash. Tested at iter 106. (priority: high)
...
- [x] [qa/P2] CLI error handling leaks stack traces — `aloop setup --autonomy-level invalid`, `aloop start` (no config), `aloop orchestrate --autonomy-level foo`, `aloop resolve --project-root /nonexistent` should return clean user-facing errors. (priority: medium)
  - Re-test 2026-03-16 (835c6fa): `aloop start --max-iterations 1` in fresh project (without setup) still prints raw JS stack trace for "No Aloop configuration found".
