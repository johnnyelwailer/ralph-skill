...
- [ ] [qa/P1] `aloop devcontainer` omits `OPENCODE_API_KEY` from `remoteEnv` and opencode CLI install from `postCreateCommand` even when opencode is explicitly configured as a provider (`--providers "claude,opencode"`). The devcontainer/P1 task claims this was added but the packaged install output does not include it. Tested at iter 171. (priority: high)
