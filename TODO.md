...
### Up Next
- [ ] [review] Gate 3: `selfHealKnownBlockers` branch coverage ~72–75% — add tests for 4 uncovered branches: (1) `repo === null` when a label blocker is detected (line ~5494: outer `if (repo)` false path silently skips without logging — add test asserting no results returned when filter_repo is null); (2) `labelMatch === null` — description contains "label" but regex extracts nothing (line ~5492: no label name extractable — add test asserting no create_github_label result); (3) non-403 `execGh` failure — create_github_label result with `success: false` but NO permission log entry (line ~5505–5513: add test asserting exactly one result, action=create_github_label, success=false, no log_permission_error); (4) `writeFile` throws during config derivation (line ~5560: deps.writeFile mock throws → derive_config_json result with success=false). All in `orchestrate.test.ts` `selfHealKnownBlockers` suite. (priority: medium)

### Completed
- [x] Self-healing for known blockers — spec §1057–1061: (a) missing GitHub labels → call `gh label create` automatically, (b) missing `config.json` → derive from `meta.json`+`orchestrator.json`, (c) permission errors → log specific permission needed. Implemented `selfHealKnownBlockers` function with `SelfHealResult` interface, integrated into `runOrchestratorScanPass` section 7.6 before diagnostics write, added 7 unit tests. All actions are best-effort with `self_heal_attempt` log entries. (priority: medium) [reviewed: gates 1-2,4-10 pass; gate 3 fail — see review task]
