...
- [ ] [review] Gate 3: `github-monitor.ts` branch coverage at **69.83%** (target >=90%). Module IS imported by `orchestrate.ts` — production code. Uncovered branches: `EtagCache.save()` mkdir guard (line 106), `EtagCache.set` data path (lines 121-125), and multiple conditionals in `ghApiWithEtag` response parsing and `fetchBulkIssueState` GraphQL response traversal. Add tests for: save to non-existent directory, `\n\n` (non-CRLF) header separator, 304 with no cached entry, response without JSON body, and nodes with missing `number` field. (priority: high)
