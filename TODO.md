...
- [ ] Fix Gate 1(a): Remove `PROMPT_cleanup.md` from the finalizer list in `.aloop/pipeline.yml` and delete `aloop/templates/PROMPT_cleanup.md`. The file was added out of scope with no orchestrator authorization; removing it from the finalizer reverts the unauthorized scope expansion. CONSTITUTION rule #12 violation — spec rewrite + feature bundled in one commit. (priority: high)
