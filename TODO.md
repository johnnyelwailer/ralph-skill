...
- [x] [review] Gate 6 / [qa/P1] Proof screenshots invalid — proof-artifacts/ contains 8 story screenshots that are all identical (MD5: 99b13def98aa849306b4f00e23948c59, size 5199 bytes each), all showing "Not found" text; proof agent used file:// protocol which 404s. Fix: run `npx storybook build`, serve via `python3 -m http.server <port>` or `npx serve`, then capture screenshots with Playwright for ProviderHealth/AllHealthy, Mixed, AllFailed; CostDisplay/NoBudgetCap, WithBudgetWarning, WithBudgetCritical; ArtifactViewer/SingleImage, WithDiffBadgeCritical; write proof-manifest.json to session artifacts dir `/home/pj/.aloop/sessions/orchestrator-20260321-172932-issue-183-20260324-085402/artifacts/` (priority: high)
- [ ] [review] Gate 2: `lib/ansi.test.ts` has 7 `.toBeTruthy()` shallow checks on ANSI color RGB values — these pass even if the color computation is wrong; rewrite each to assert the exact string value: line 76 `segments[1].style.fg` should be `'187,0,0'` (ANSI 31); line 115 `segments[0].style.fg` for `38;5;200` should be `'255,0,215'`; line 119 `segments[0].style.bg` for `48;5;100` should be `'175,135,0'`; line 145 `segments[1].style.fg` should be `'187,0,0'` (ANSI 31); line 146 `segments[1].style.bg` should be `'187,0,0'` (ANSI 41); line 172 `segments[1].style.fg` should be `'85,85,85'` (ANSI 90); line 173 `segments[1].style.bg` should be `'85,85,85'` (ANSI 100) (priority: high)
- [x] [review] Gate 4: REVIEW_LOG.md was partially restored (contains reconstructed 2fbd29ee FAIL and the current FAIL entry) but is still missing the PASS entry at b0cf335a — prepend the b0cf335a PASS entry to REVIEW_LOG.md; log is append-only so only add what is missing (priority: high) [reviewed: gates 1-9 pass]
...
- [x] Extract `lib/ansi.ts` — move `stripAnsi`, `PALETTE_256`, `rgbStr`, `parseAnsiSegments`, `renderAnsiToHtml` out of `AppView.tsx` into `lib/ansi.ts`; update all imports; add unit tests in `lib/ansi.test.ts` (priority: high, foundational — blocks AnsiRenderer) [reviewed: Gate 2 fail — shallow toBeTruthy() checks on color values, fix tasks written]
