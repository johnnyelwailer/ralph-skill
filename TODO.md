...
- [ ] [spec-gap/P2] Tablet sidebar shows collapsed icon-strip instead of "hidden": Spec (SPEC.md line 21, SPEC-ADDENDUM.md line 214) says "Tablet (640-1024px): sidebar collapsible, **hidden by default**". Code renders a 40px collapsed icon strip (AppView.tsx:2467 `hidden sm:flex` always renders, AppView.tsx:2472 `collapsed={isDesktop ? false : !sidebarOpen}` → collapsed=true but not hidden). The proof-manifest.json itself notes "sidebar always visible via sm:flex" at tablet. Decision needed: either make the tablet sidebar fully hidden behind a hamburger (like mobile but without overlay), or update spec to reflect the collapsed-icon-strip design as intentional. P2 — correctness drift, not a runtime failure.

- [ ] [spec-gap/P3] Ctrl+B works on mobile as well as tablet (over-implementation): Spec says "Ctrl+B keyboard shortcut for sidebar toggle at tablet breakpoint" (SPEC.md line 29). Code at AppView.tsx:2189 calls `toggleSidebar()` for any non-desktop breakpoint, including mobile. Low priority — additional functionality, not missing functionality.
