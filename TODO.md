...
### Up Next
- [ ] [known-issues/P1] Normalize line endings during `install.ps1` runtime copy (`loop.ps1` as CRLF, `loop.sh` as LF). Currently `Copy-Item` and `Set-Content` use platform defaults with no explicit encoding/EOL control. (priority: high, prevents stale/corrupt installed runtime)
