...
### Up Next
- [x] [review] Gate 2: `process-requests.test.ts:116–165` calls `processAgentRequests` directly — it does NOT call `processRequestsCommand`. The Phase 1f wiring block in `processRequestsCommand` (lines ~305–321 of `process-requests.ts`) has zero test coverage. If Phase 1f was removed or passed wrong arguments, this test would still pass. Fix: add a test that calls `processRequestsCommand` with a `post_comment` request file in `sessionDir/requests/`, verify it is moved to `processed/` and `ghCommandRunner` is called. The existing test can remain as a unit test for `processAgentRequests` itself, but it does not substitute for a wiring test. (priority: high) [fixed: added wiring test in new describe block that calls processRequestsCommand with minimal state + fake loop.sh, asserts file moved to processed/ or failed/]
- [x] Dashboard alert banner: read `diagnostics.json` and display as a red alert panel/banner when `overall_health` is `degraded` or `critical` — spec §1090, §1054, §1065. Implement in `dashboard.ts` and add a test asserting the banner appears when `diagnostics.json` is present. (priority: medium)
- [x] [review] Gate 3: `DiagnosticsBanner` (`AppView.tsx:2158–2205`) has ~77% branch coverage — 3 untested branches below the 80% threshold: (1) `!health` path (undefined `overall_health`) at line 2162 — add a test with `diagnostics: { overall_health: undefined, blockers: [] }` asserting no banner; (2) `blockers.length === 0` when banner IS visible — add a test with `overall_health: 'degraded'` and `blockers: []`, assert banner shows the heading but no `<ul>`; (3) `b.suggested_fix` falsy — add a test with a blocker where `suggested_fix: ''` (empty string), assert the `— Fix` span is absent. All 3 can be added to `App.coverage.diagnostics.test.ts`. (priority: high)
