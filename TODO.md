...
- [ ] [qa/P1] [bug] `aloop start` stale runtime warning compares against the current project git repo instead of aloop runtime source metadata. Reproduced in packaged-install QA on 2026-03-19 (iter 253). (priority: high)
...
- [ ] [qa/P1] [bug] `single` mode parity gap: added `'single'` to `start.ts:20` `LOOP_MODE_SET` and extended `compile-loop-plan` cycle support + regression tests. QA tested at iter 251; re-tested 2026-03-19 (iter 253) and still failing with `Error: Invalid mode: single` in packaged install. (priority: high)
