...
- [ ] [qa/P1] process-requests does not archive request files: `aloop process-requests --session-dir <dir>` exits 0 silently but leaves all request files in `requests/` — neither valid requests move to `requests/processed/` nor malformed JSON to `requests/failed/`. Reproduced with `orchestrator.json` present (required for dispatch_child), correct request format `{"id":"..","type":"dispatch_child","payload":{..}}`, and real JSON files in `requests/`. Spec says "Request files archived after processing (not deleted)" and "Malformed requests moved to requests/failed/ with error annotation". Separately, `process-requests.test.ts` fails to load: `SyntaxError: Export named 'syncMasterToTrunk' not found in module process-requests.ts`. Tested at iter 1. (priority: high)
