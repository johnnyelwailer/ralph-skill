...
### Up Next
- [ ] [review] Gate 2: `process-requests.test.ts:116–165` calls `processAgentRequests` directly — it does NOT call `processRequestsCommand`. The Phase 1f wiring block in `processRequestsCommand` (lines ~305–321 of `process-requests.ts`) has zero test coverage. If Phase 1f was removed or passed wrong arguments, this test would still pass. Fix: add a test that calls `processRequestsCommand` with a `post_comment` request file in `sessionDir/requests/`, verify it is moved to `processed/` and `ghCommandRunner` is called. The existing test can remain as a unit test for `processAgentRequests` itself, but it does not substitute for a wiring test. (priority: high)
