...
- [ ] [review] Gate 1: `evaluatePolicy` in `gh.ts` uses `payload.repo || 'owner/repo'` placeholder — never loads session config. Implement hard enforcement: load session repo from config, reject requests with mismatched repo. (priority: high)
