...
- [ ] [review] Gate 2: No tests exist for the final-review invariant implemented in `aloop/bin/loop.ps1` and `aloop/bin/loop.sh`. All 7 spec acceptance criteria (forced review after tasks complete, review-approval exit, rejection/replan, steering priority, log events) are completely untested. The pending task "Add focused loop regression tests..." must be completed before this work is considered done. Write subprocess-based tests (Pester or Node) covering: (1) plan-build-review exits only via forced review approval, not during build phase; (2) review rejection reopens tasks and forces re-plan; (3) steering mid-flight clears allTasksMarkedDone and does not trigger stray review; (4) tasks_marked_complete/final_review_approved/final_review_rejected events appear in log.jsonl. (priority: high)
- [ ] [review] Gate 3: `aloop/bin/loop.ps1` new final-review logic (lines ~755–846, ~22 new lines) has 0% branch coverage — the `plan-build-review` task-complete branch, `final_review_approved` exit, and `final_review_rejected`/replan branch are never exercised by any test. `aloop/bin/loop.sh` new logic (lines ~660–739, ~44 new lines) likewise has 0% coverage. Both must reach ≥80% branch coverage before the gate passes. (priority: high)
