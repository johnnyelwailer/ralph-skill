...
### Up Next
- [ ] [cleanup/P1] Remove hallucinated `single` mode — QA agent fabricated a mode not in SPEC.md; build agent then implemented it. Revert all `single` mode code from: `start.ts` (type + validation set), `compile-loop-plan.ts` (type + switch case), `loop.sh` (help text + resolve case), `loop.ps1` (ValidateSet + handling), and their test files (`start.test.ts`, `compile-loop-plan.test.ts`, `loop.tests.ps1`). 10 files contaminated. (priority: high)
...
- [x] [qa/P1] [bug] `single` mode parity gap: added `'single'` to `start.ts:20` `LOOP_MODE_SET` and extended `compile-loop-plan` cycle support + regression tests. QA tested at iter 251. (priority: high)
