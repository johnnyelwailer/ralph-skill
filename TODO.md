...
### In Progress
- [ ] [review] Gate 2: `aloop/cli/src/commands/project.test.ts:106-107` uses existence-only assertions (`existsSync`) for orchestrate prompt generation; strengthen this test to assert concrete generated prompt content (or exact prompt set) so it fails on wrong/partial copies, not just missing files. (priority: high)
