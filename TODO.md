...
- [ ] [review] Gate 2: `MainPanel.test.tsx:79` — `it('calls setActivityCollapsed when collapse button clicked')` never clicks the collapse button and never asserts the mock was called — a broken collapse handler passes this test. Fix: click `screen.getByLabelText('Collapse activity panel')` and assert `expect(setActivityCollapsed).toHaveBeenCalledWith(true)`. (priority: high)
- [ ] [review] Gate 2: `DocsPanel.test.tsx:47` — `it('switches tab when tab trigger is clicked')` only asserts the tab count is still 4 after clicking tab[1] — does not verify the active tab changed or that SPEC content appeared. Fix: after clicking the SPEC tab, assert `screen.getByRole('tab', { name: 'SPEC' })` has `data-state="active"` or that SPEC-specific content is in the DOM. (priority: high)
- [ ] [review] Gate 2: `DocsPanel.test.tsx:63` — `it('switches to health tab')` clicks the health tab inside `waitFor` but makes no post-click assertion — a broken click handler passes. Fix: after clicking, assert health-panel-specific content is visible (e.g. `expect(screen.getByText(/Provider Health/i)).toBeInTheDocument()` or similar rendered output from `HealthPanel`). (priority: high)
