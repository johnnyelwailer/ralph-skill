...
- [x] [review] Gate 3: `QACoverageBadge.tsx` still has 2 untested branches — new-module ≥90% threshold not fully met. Add to `QACoverageBadge.test.tsx`: (1) `!response.ok` early return (line 69) — render with `mockFetch(data, false)` (pass `ok=false` second arg) and assert the component renders `null` (no button rendered after waiting); (2) `sessionId={null}` → sp='' path (line 67) — render `<QACoverageBadge sessionId={null} refreshKey="k1" />` with a `vi.spyOn(global, 'fetch')` and assert the fetch URL argument contains no `?session=` substring. (priority: high)
...
- [x] [review] Gate 3: `QACoverageBadge.tsx` expansion panel branch coverage — added 6 tests: click toggles expand (ChevronDown appears), empty features → "No feature rows found", PASS/FAIL/UNTESTED feature rows with correct tone classes, empty component string omits `<p>`. All 11 QACoverageBadge tests pass.
