...
- [x] Add tests for steering reset of cyclePosition in `aloop/bin/loop_branch_coverage.tests.sh` — cover: (1) after steering queue item executes successfully, `CYCLE_POSITION` is 0; (2) non-steering queue item does NOT reset `CYCLE_POSITION`. Register branch IDs: `queue.steer_reset`, `queue.nonsteer_no_reset`. [qa re-test iter 3: confirmed still absent — no queue.steer_* branches in coverage JSON. Acceptance criteria gap.]

- [x] [review] Gate 1 + Gate 3: Steering cyclePosition reset has no automated test coverage — `queue.steer_reset` and `queue.nonsteer_no_reset` branch IDs are not registered or covered in `aloop/bin/loop_branch_coverage.tests.sh`. SPEC acceptance criterion requires automated coverage for each new behavior including steering reset. Add two test cases to the branch-sync section of loop_branch_coverage.tests.sh: (1) mock a `*-PROMPT_steer.md` queue item through `run_queue_if_present` and assert `CYCLE_POSITION=0` after success; (2) mock a non-steering queue item and assert `CYCLE_POSITION` is unchanged. Register `queue.steer_reset` and `queue.nonsteer_no_reset`. Note: run_queue_if_present invokes invoke_provider so tests will need the same mock infrastructure as existing queue tests. (priority: high)

- [x] [qa/P1] Gate 4 incomplete — dist/index.js still has 1575 lines of uncommitted out-of-scope changes in working tree: LOOP_PROMPT_TEMPLATES expanded from 6 to 11 entries, plus pipeline.yml scaffolding additions. The Gate 4 revert commit (754943cc) only changed TODO.md; `git checkout -- aloop/cli/dist/index.js` was NOT effectively applied. Run `git checkout -- aloop/cli/dist/index.js` to discard the working tree modification, then verify git status is clean. Tested at iter 5. (priority: high)
