...
- [ ] [qa/P1] Packaged install template bootstrap still broken (`aloop scaffold` / `aloop setup --non-interactive --mode loop`) — fresh `HOME` run still crashes with `Template not found: .../.aloop/templates/PROMPT_plan.md` and raw JS stack frames in packaged binary path (`/tmp/aloop-test-install-*/bin/aloop`). Tested at iter current. (priority: high)
- [ ] [qa/P1] `aloop dashboard` packaged-install asset resolution broken in fresh project — running `aloop dashboard --port 4141` serves fallback HTML (`Dashboard assets not found at <project>/dashboard/dist`) instead of the dashboard UI; Playwright 1920x1080 check reports `visiblePanels=0`. Tested at iter current. (priority: high)
