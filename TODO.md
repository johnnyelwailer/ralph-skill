...
- [ ] [review] Gate 3: `github-monitor.ts` is a new module at **69.83% branch coverage** (target >=90%). Uncovered branches: `EtagCache.save()` mkdir guard (line 106), `EtagCache.set` data path (lines 121-125), and multiple conditionals in `ghApiWithEtag` response parsing and `fetchBulkIssueState` GraphQL response traversal. Add tests for: save to non-existent directory, `\n\n` (non-CRLF) header separator, 304 with no cached entry, response without JSON body, and nodes with missing `number` field. (priority: high)
- [ ] [review] Gate 3: `github-webhook.ts` is a new module at **75.38% branch coverage** (target >=90%). Uncovered branches: `WebhookServer.start()` re-entry guard (line 98), `server.address()` type check (lines 108-116), `stop()` when already stopped, and handler error path (line 176). Add tests for: double-start no-op, double-stop no-op, handler that throws, and request with invalid JSON body. (priority: high)
- [ ] [review] Gate 4: `github-webhook.ts` is **dead code** — not imported by any production module. Only `github-webhook.test.ts` references it; `orchestrate.ts` imports only from `github-monitor.ts`. Either wire `WebhookServer`/`isRelevantWebhookEvent`/`extractInvalidationKey` into the orchestrator scan loop (per SPEC step 0.3 webhook push), or remove the module until it's needed. (priority: high)
### Up Next — P1 Hardening
- [ ] [qa/P1] `aloop start` fails after fresh packaged-install setup in isolated `HOME`: `Error: Loop script not found: <HOME>/.aloop/bin/loop.sh`. Repro: fresh `HOME` → `aloop setup --non-interactive --mode loop --providers copilot --spec SPEC.md` (exit 0) → `aloop start --max-iterations 1` (exit 1). Spec UX contract says `aloop start` handles full session setup + loop launch; this blocks loop-mode start in clean environments. Tested at iter 174. (priority: high)
