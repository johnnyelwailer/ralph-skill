...
- [x] **setup-discovery.ps1: scaffold must copy PROMPT_steer.md** — The scaffold function (line 574) only generates plan/build/review prompts. The steer template is never copied to `~/.ralph/projects/<hash>/prompts/`, so when `start.md` copies prompts to the session directory, `PROMPT_steer.md` is missing. The loop detects `STEERING.md` but warns "PROMPT_steer.md is missing" and skips steering entirely. Fix: add `'steer'` to the `@('plan', 'build', 'review')` foreach loop, but note the steer template has no project-specific variables to substitute (no `{{VALIDATION_COMMANDS}}` etc.), so it can be a straight copy from `~/.ralph/templates/PROMPT_steer.md`. (priority: critical — steering is completely broken)
...
- [ ] **Copilot: add ralph-steer.prompt.md** — Claude/Codex have `steer.md` as a slash command but Copilot has no `ralph-steer.prompt.md`. Users on Copilot cannot steer a running loop from VS Code. Add `copilot/prompts/ralph-steer.prompt.md` mirroring the Claude steer command. Also update `install.ps1` summary output if needed. (priority: medium — feature parity)

- [ ] **Extract Show-CheckboxMenu into shared module** — `install.ps1` (line 222) and `uninstall.ps1` (line 24) contain identical ~90-line `Show-CheckboxMenu` function. Extract to `ralph/bin/ui-helpers.ps1` and dot-source it from both scripts. Reduces maintenance burden when the UI changes. (priority: low — code quality)
