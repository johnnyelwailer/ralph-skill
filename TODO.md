...
### In Progress
- [ ] [review] Gate 2+3: Add `SessionCard.test.tsx` — SessionCard.tsx has no unit tests. All other extracted components in this PR have test files (StatusDot.test.tsx, PhaseBadge.test.tsx, ElapsedTimer.test.tsx). Test the key branches: suppressClick=true skips onSelect and calls onClearSuppressClick; costUnavailable shows "Cost: unavailable" in tooltip; cardCost number formats correctly; stuckCount>0 shows red stuck line (priority: high)
