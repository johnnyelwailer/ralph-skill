...
- [ ] [review] Gate 2: `Header.test.tsx:94-97` — `renders connection indicator` only asserts `data-testid="session-header-grid"` exists; this is the outer wrapper div, not the ConnectionIndicator component. A broken ConnectionIndicator wouldn't fail this test. Rewrite to assert ConnectionIndicator renders — e.g. locate the indicator element by its `data-testid` or accessible label and check its content/class matches `connectionStatus`. (priority: high)
