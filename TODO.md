...
- [x] [orchestrator/P2] Efficient GitHub monitoring — ETag-guarded REST change detection, targeted GraphQL full-state fetch, `since` filtering, optional webhook push. Implemented: `github-monitor.ts` with EtagCache (file-persisted, prefix invalidation, TTL freshness), `ghApiWithEtag` wrapper (conditional requests via `If-None-Match`, `gh api --include` parsing), `fetchBulkIssueState` (single GraphQL query for issues + PRs + check runs + project status), `detectIssueChanges` for change detection; `github-webhook.ts` with WebhookServer (HMAC signature verification, event parsing, relevance filtering, cache invalidation keys); integrated into orchestrate scan loop as step 0.3 with ETag cache persistence. 48 new tests (28 monitor + 20 webhook). (priority: medium)
- [x] [orchestrator/P2] Devcontainer routing — per-task `sandbox: container|none`, `requires: [windows, macos, gpu]`, dispatcher host-capability checks. Implemented per-issue `sandbox`/`requires` fields in decomposition/state models, host-capability detection/filtering (`windows`, `macos`, `linux`, `gpu`, `docker`, `network-access`) in dispatcher paths, skip+log of unsatisfied requirement issues during scan dispatch, and regression tests for capability filtering/dispatch skip behavior. (priority: medium)
