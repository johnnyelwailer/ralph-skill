...
Read `<runtime_root>/sessions/<session-id>/status.json` and `meta.json` for the chosen session.
...
Check if `<runtime_root>/sessions/<session-id>/STEERING.md` already exists (a previous instruction not yet processed).
...
Write `<runtime_root>/sessions/<session-id>/STEERING.md` (overwrite or append per Step 4 choice):
...
<notes>
- Write STEERING.md to the SESSION directory (`<runtime_root>/sessions/<id>/`), not the project work directory. The loop copies it to WorkDir just before invoking the spec-update agent.
