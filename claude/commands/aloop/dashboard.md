...
<objective>
Launch the Aloop dashboard by discovering active sessions and delegating to `aloop dashboard`.
</objective>
...
Any user-provided flags after `/$skillName:dashboard` are passed through:
- `--port <n>` -> `--port <n>`
